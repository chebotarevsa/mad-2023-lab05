...
import com.example.anki.adapters.ActionInterface
import com.example.anki.adapters.CustomRecyclerAdapter
import com.example.anki.models.Model
import com.example.anki.databinding.FragmentListCardBinding
...
class ListCardFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.addButton.setOnClickListener {
            val action =
                com.example.anki.fragments.ListCardFragmentDirections.actionListCardFragmentToManageCardFragmentAdd()
    ...
    private val action = object : ActionInterface {
        override fun onItemClick(cardId: Int) {
            val action =
                com.example.anki.fragments.ListCardFragmentDirections.actionListCardFragmentToSeeCardFragment(
                    cardId
                )
            findNavController().navigate(action)
        }
        ...
        override fun onDeleteCard(cardId: Int) {
            ...
            AlertDialog.Builder(requireContext()).setIcon(android.R.drawable.ic_dialog_alert)
                ...
                .setPositiveButton("Да") { _, _ ->
                    ...
                    Toast.makeText(
                        requireContext(), "Все карточки на месте!", Toast.LENGTH_LONG
                    ).show()
                }.show()
        }
    }
